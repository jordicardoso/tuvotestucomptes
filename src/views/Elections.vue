    ...
    <v-layout column>
      <v-card>
        ...
        <v-list two-line subheader>
          <v-list-tile v-for="eleccio in eleccions" :key="eleccio.Desc" @click="openPartits(eleccio)">
            <v-list-tile-content>
              <v-list-tile-title class="text--primary">{{ eleccio.Desc }}</v-list-tile-title>
      ...
      <v-card color="blue-grey darken-2" class="white--text mt-3" v-if="eleccio">
        ...
        <v-list two-line subheader>
          <v-list-tile v-for="partit in partits" :key="partit.Nomcurt">
            <v-list-tile-content>
              <v-list-tile-action-text class="text--primary">{{ partit.Nomcurt }}</v-list-tile-action-text>
...
export default {
  ...
  filters: {
    getDate: function (value) {
      ...
      let month = outDate.toDate().getMonth()
  ...
  methods: {
    openPartits (eleccio) {
      ...
      var self = this
      db.collection('eleccions').doc('K92HNeLF7hsBDA1KntKB').collection('partits').orderBy('Nomcurt')
        .get()
